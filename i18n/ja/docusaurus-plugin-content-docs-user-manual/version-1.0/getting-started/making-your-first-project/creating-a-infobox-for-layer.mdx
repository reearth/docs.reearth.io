...
地図上にマーカーを配置した後、そのマーカーに関する詳細情報を表示したい場合に、[インフォボックス]( /user-manual/1.0/infobox/overview)は最適なツールです。
...
これで空のインフォボックスができましたが、これに情報を追加する場合は、[ブロック]( /user-manual/1.0/infobox/add-a-new-block)を作成しなければなりません。ここでは、例としてテキストブロックを作成します。
...
Re:Earthでは、インフォボックスを通じてさまざまなレイヤーに情報を追加しています。この方法の詳細については、[インフォボックスのセクション]( /user-manual/1.0/infobox/overview)を参照してください。
